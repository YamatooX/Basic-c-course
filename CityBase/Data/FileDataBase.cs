    ...
    public class FileDataBase : IDataBase
    {
        ...
        private async Task UpdateFile()
        {
            using (StreamWriter writer = new StreamWriter(_path))
            {
                ...
                {
                    writer.WriteLine(HandleToWrite(estate));
                }
            }
        }

        private async Task UpdateEstate(Estate estate)
        {
        ...
        private string HandleToWrite(Estate estate)
        {
        ...
        private Office ReadOffice(string[] line)
        {
            ...
            Property property;
            if (!Enum.TryParse<Property>(line[2], out property))
            {
                Console.WriteLine("Incorrect property");
            }
            ...
            if (!int.TryParse(line[8], out maxCapacity))
            {
                Console.WriteLine("Incorrect capacity");
            }

            DateTime addedDate;
            if(!DateTime.TryParse(line[9], out addedDate)){
        ...
        private Parcel ReadParcel(string[] line)
        {
            ...
            ParcelType type;
            if (!Enum.TryParse<ParcelType>(line[7], out type))
            {
                Console.WriteLine("Incorrect type");
            }
            DateTime addedDate;
            if (!DateTime.TryParse(line[8], out addedDate))
            {
                Console.WriteLine("Incorrect date");
            }
        ...
        //private void NewUpdateFile()
        //{
        //    // Wyczyścić całą listę i dodać ją na nowo do pliku (?)
        //    for(int i = 1; i <= _dataBase.Count; i++)
        //    {
        //        Estate e1 = _dataBase[i];
        //        if (_dataBase[i + 1] == null)
        //        {
        //            return;
        //        }
        //        Estate e2 = _dataBase[i + 1]; // To do => handle out of range exception

        //        if(e2.Id - e1.Id == 2)
        //        {
        //            // dodaj estate większy od e1 => Tu można wrzucać nowy estate => Check If Is Free Space(?) <= Nowa metoda, wszystko mogłoby dziać się automagicznie?
        //            // Albo zmniejsz Id 2 o 1 i zwrócić nową listę => Mało wydajne
        //        }
        //    }
        //}
    }
